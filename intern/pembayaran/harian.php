<?php
    ...
    function BrivaUpdate($client_id,$secret_id,$token, $id_siswa, $jenis, $verb){
        ...
        //print_r($jsonPost);
        $datak=array();
        foreach($jsonPost['data'] as $mydata){
            //echo $mydata['nama'] . "\n";
            $datax['nama'][]=$mydata['nama'];
            $datax['keterangan'][]=$mydata['keterangan'];
            $datax['amount'][]=$mydata['amount'];
            $datax['paymentDate'][]=$mydata['paymentDate'];
            $datax['no_rek'][]=$mydata['no_rek'];
        }
        return $datax;
    }
    ...
    $pesan='';
    $status=true;
    $list=BrivaUpdate($clientid,$clientsecret,$token,'','1',"GET");
